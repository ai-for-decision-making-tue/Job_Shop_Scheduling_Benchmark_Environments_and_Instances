...
for i, wos in enumerate(frappe.get_all("Work Order")):
    ...
    for j, ops in enumerate(wo['operations']):
        ...
        operation = {"operation_id": number_operations, "processing_times": {}, "predecessor": _pred}
        number_operations += 1
...
processing_info = {
    ...
    f"machine_{i+1}": np.zeros((number_operations, number_operations), dtype=int).tolist()
...
print(test_wkn)
import json
with open(r'/home/cole/cole_scripts/job_scheduling_env/processing_info.json', 'w') as file:
...
if OUTPUT is True:
    ...
    import pickle
    with open(r'/home/cole/cole_scripts/job_scheduling_env/jobShopEnv.pkl', 'wb') as file:
