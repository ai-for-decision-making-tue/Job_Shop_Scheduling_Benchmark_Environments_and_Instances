...
from solution_methods.FJSP_DRL.online_dispatching_helpers import (
    load_dispatcher_FJSP_DRL, schedule_operations_drl)
...
def run_method(simulationEnv, dispatching_rule, machine_assignment_rule):
    ...
    if dispatching_rule == "FJSP_DRL":
        dispatcher = load_dispatcher_FJSP_DRL()
    if simulationEnv.online_arrivals:
        ...
        while True:
            if dispatching_rule == "FJSP_DRL":
                schedule_operations_drl(simulationEnv, dispatcher)
            else:
                schedule_operations(
                    simulationEnv, dispatching_rule, machine_assignment_rule
                )
        ...
        if dispatching_rule == "FJSP_DRL":
            raise ValueError(
                "FJSP_DRL can only be used with online arrivals. For offline scheduling, use the dedicated FJSP_DRL script."
            )
    ...
    if not parameters["instance"]["online_arrivals"]:
        try:
            if "fjsp" in parameters["instance"]["problem_instance"]:
                simulationEnv.JobShop = parser_fjsp.parse(
                    simulationEnv.JobShop, parameters["instance"]["problem_instance"]
                )
            elif "fajsp" in parameters["instance"]["problem_instance"]:
                simulationEnv.JobShop = parser_fajsp.parse(
                    simulationEnv.JobShop, parameters["instance"]["problem_instance"]
                )
            elif "jsp" in parameters["instance"]["problem_instance"]:
                simulationEnv.JobShop = parser_jsp_fsp.parse(
                    simulationEnv.JobShop, parameters["instance"]["problem_instance"]
                )
        ...
        simulationEnv.simulator.process(
            run_method(
                simulationEnv,
                parameters["instance"]["dispatching_rule"],
        ...
        simulationEnv.JobShop.set_nr_of_machines(
