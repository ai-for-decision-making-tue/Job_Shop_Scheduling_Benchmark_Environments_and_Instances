...
def permissibleLeftShift(a, durMat, mchMat, mchsStartTimes, opIDsOnMchs):
    ...
    # print('JobReadyTime:', jobRdyTime_a, 'MachineReadyTime:', mchRdyTime_a, 'possiblePos:', possiblePos, 'machine_start_times:', startTimesForMchOfa)
    if len(possiblePos) == 0:
        startTime_a = putInTheEnd(a, jobRdyTime_a, mchRdyTime_a, startTimesForMchOfa, opsIDsForMchOfa)
    else:
        idxLegalPos, legalPos, endTimesForPossiblePos, gaps, startTimesOfPossiblePos, startTimePredecessor, durTimePredecessor, durOfPossiblePos \
            = calLegalPos(dur_a, jobRdyTime_a, durMat, possiblePos, startTimesForMchOfa, opsIDsForMchOfa)
        # print('legalPos:', legalPos, 'duration:', dur_a, 'possibleGaps:', gaps, 'startTimesOfPossiblePos',
        #       startTimesOfPossiblePos, 'endTimesForPossiblePos:' , endTimesForPossiblePos, 'startTimePredecessor:',
        #       startTimePredecessor, 'durTimePredecessor:', durTimePredecessor, 'durOfPossiblePos', durOfPossiblePos)
        if len(legalPos) == 0:
            startTime_a = putInTheEnd(a, jobRdyTime_a, mchRdyTime_a, startTimesForMchOfa, opsIDsForMchOfa)
        else:
            flag = True
...
def calLegalPos(dur_a, jobRdyTime_a, durMat, possiblePos, startTimesForMchOfa, opsIDsForMchOfa):
    ...
    startTimePredecessor = startTimesForMchOfa[possiblePos[0]-1]
    durTimePredecessor = np.take(durMat, [opsIDsForMchOfa[possiblePos[0]-1]])
    ...
    return idxLegalPos, legalPos, endTimesForPossiblePos, possibleGaps, startTimesOfPossiblePos, startTimePredecessor, durTimePredecessor, durOfPossiblePos
