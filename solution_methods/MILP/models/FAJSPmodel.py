...
# Customized to work with assembly constraints (see instances of FAJSP dataset).
import re
...
def fajsp_milp(jobShopEnv, time_limit):
    ...
    #  Custom added constraint: Precedence constraints among operations of different jobs (assembly operations)
    for j in jobs:
        JOB = jobShopEnv.get_job(j)
        for l in operations_per_job[j]:
            OPERATION = jobShopEnv.get_operation(l)
            for predecessor in OPERATION.predecessors:
                predecessor_job_id = predecessor.job_id
                predecessor_task_id = predecessor.operation_id
                if JOB != predecessor_job_id:
                    lhs = quicksum(S[j, l, i] for i in machine_allocations[j, l])
                    rhs = quicksum(S[predecessor_job_id, predecessor_task_id, i] for i in machine_allocations[predecessor_job_id, predecessor_task_id]) \
                          + quicksum(operations_times[predecessor_job_id, predecessor_task_id, i] * Y[predecessor_job_id, predecessor_task_id, i]
                        for i in machine_allocations[predecessor_job_id, predecessor_task_id]
                    )
                    model.addConstr(lhs >= rhs)
